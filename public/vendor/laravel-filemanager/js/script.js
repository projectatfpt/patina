...
function toggleActions() {
    ...
    $("[data-action=preview]").toggleClass(
        "d-none",
        !(many_selected && only_file)
    );
    $("[data-action=move]").toggleClass("d-none", !many_selected);
    $("[data-action=download]").toggleClass(
        "d-none",
        !(many_selected && only_file)
    );
    $("[data-action=resize]").toggleClass(
        "d-none",
        !(one_selected && only_image)
    );
    $("[data-action=crop]").toggleClass(
        "d-none",
        !(one_selected && only_image)
    );
...
function loadItems(page) {
    ...
    performLfmRequest(
        ...
        $("#show_tree")
            .toggleClass("d-none", !atRootFolder)
            .toggleClass("d-block", atRootFolder);
        setOpenFolders();
        loading(false);
        toggleActions();
    });
...
function use(items) {
    ...
    if (usingWysiwygEditor()) {
        ...
        // standalone button or other situations
...
// Ví dụ: Sử dụng các hàm này trong các tình huống cụ thể
document.addEventListener("DOMContentLoaded", function () {
    showLoading(); // Hiển thị loading khi trang bắt đầu tải

    // Giả lập việc tải dữ liệu
    setTimeout(function () {
        hideLoading(); // Ẩn loading sau 3 giây
    }, 3000);
});
